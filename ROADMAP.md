...
| Federation vocabulary | Partial | `spec/federation.md` defines the portability firewall, issuer metadata document, trust roots, identity attestations (`ed25519-jcs` + JWT), and key discovery; `sdk-js/src/federation/` is a working reference adapter with tests | Freeze the attestation/issuer-metadata wire shapes; add attestation vectors and negative cases; specify profile discovery/negotiation |
